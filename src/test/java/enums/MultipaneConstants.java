package enums;

import lombok.AllArgsConstructor;
import lombok.Getter;

@Getter
@AllArgsConstructor
public enum MultipaneConstants {

    ONE_GEO_IS_INCLUDED("1 geo(s) are included"),
    ONE_MEDIA_IS_INCLUDED("1 media are included"),
    ONE_AD_SIZE_IS_INCLUDED("1 size(s) are included"),
    ONE_DEVICE_IS_INCLUDED("1 device(s) are included"),
    ONE_AD_FORMAT_IS_INCLUDED("1 format(s) are included"),
    ONE_DEMAND_SOURCE_IS_INCLUDED("1 demand source(s) are included"),
    ONE_OPERATING_SYSTEM_IS_INCLUDED("1 operating system(s) are included");

    private final String value;
}
